...
$CLI --profile postgres --config mariadb --action addPluginConfig --config-file $CONFIG_DIR/maraidb/mariadb/aws.so.conf --plugin aws.so --credential-file $CREDENTIAL_FILE 
...
$CLI --profile postgres --config mongo --action addPluginConfig --config-file $CONFIG_DIR/mongo/mongo/aws.so.conf --plugin aws.so --credential-file $CREDENTIAL_FILE 
