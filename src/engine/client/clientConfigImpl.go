...
func AddConfig(auth Auth,profileName,configName string,config util.Config) (util.Result,error) {
	...
	req, err := http.NewRequest("GET", "http://fossil-workflow:8000/addConfig/" + profileName + "/" + configName, b)
	if err != nil {
		return result,err
	}
...
func DeleteConfig(auth Auth,profileName,configName string) (util.Result,error) {
	...
	req, err := http.NewRequest("GET", "http://fossil-workflow:8000/deleteConfig/" + profileName + "/" + configName, nil)
	if err != nil {
		return result,err
	}
...
func AddProfile(auth Auth,profileName string) (util.Result,error) {
	...
	req, err := http.NewRequest("GET", "http://fossil-workflow:8000/addProfile/" + profileName, nil)
	if err != nil {
		return result,err
	}
