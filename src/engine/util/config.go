...
func GetConsolidatedConfig(configDir, profileName, configName, policyName string) (Config, error) {
	...
	backupRetention := GetBackupRetention(policyName, config.BackupRetentions)
	config.SelectedBackupRetention = backupRetention
	archiveRetention := GetArchiveRetention(policyName, config.ArchiveRetentions)
	...
	if config.AppPlugin != "" {
		...
		appConfigMap, err := ReadConfigToMap(appConf)
	...
	if config.StoragePlugin != "" {
		...
		storageConfigMap, err := ReadConfigToMap(storageConf)
	...
	if config.ArchivePlugin != "" {
		...
		archiveConfConfigMap, err := ReadConfigToMap(archiveConf)
